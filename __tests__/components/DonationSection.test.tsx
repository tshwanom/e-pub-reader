...
describe('DonationSection', () => {
  ...
  it('lets the donor switch to monthly support and keeps recurring checkout on PayPal', async () => {
    ...
    expect(screen.getByText(/Monthly recurring support is currently available through PayPal/i)).toBeInTheDocument();

    const paystackGatewayButton = screen.getAllByRole('button', { name: /Paystack/i })[0];
    expect(paystackGatewayButton).toBeDisabled();
